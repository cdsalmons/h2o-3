...
def offset_tweedie():
  ...
  assert abs(49.2298892965 - predictions.mean().getrow()[0]) < 1e-3, "expected prediction mean to be {0}, but got {1}". \
    format(49.2298892965, predictions.mean().getrow()[0])
  assert abs(1.02166639486 - predictions.min()) < 1e-4, "expected prediction min to be {0}, but got {1}". \
    format(1.02166639486, predictions.min())
  assert abs(392.904280395 - predictions.max()) < 1e-2, "expected prediction max to be {0}, but got {1}". \
    format(392.904280395, predictions.max())
