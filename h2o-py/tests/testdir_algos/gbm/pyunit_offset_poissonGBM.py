...
def offset_poisson():
    ...
    assert abs(49.23437 - predictions.mean().getrow()[0]) < 1e-4, "expected prediction mean to be {0}, but got {1}". \
        format(49.23437, predictions.mean().getrow()[0])
    assert abs(1.07214484305 - predictions.min()) < 1e-4, "expected prediction min to be {0}, but got {1}". \
        format(1.07214484305, predictions.min())
    assert abs(398.256766609 - predictions.max()) < 1e-2, "expected prediction max to be {0}, but got {1}". \
        format(398.256766609, predictions.max())
