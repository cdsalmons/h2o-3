...
def offset_tweedie():
    ...
    assert abs(49.21591 - predictions.mean().getrow()[0]) < 0.014, "expected prediction mean to be {0}, but got {1}". \
        format(49.21591, predictions.mean().getrow()[0])
    assert abs(1.0258 - predictions.min()) < 0.0042, "expected prediction min to be {0}, but got {1}". \
        format(1.0258, predictions.min())
    assert abs(392.4651 - predictions.max()) < 0.44, "expected prediction max to be {0}, but got {1}". \
