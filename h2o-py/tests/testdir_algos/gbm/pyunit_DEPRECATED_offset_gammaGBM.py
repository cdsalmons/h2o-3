...
def offset_gamma():
    ...
    assert abs(50.1584874405 - predictions.mean().getrow()[0]) < 5e-2, "expected prediction mean to be {0}, but got {1}". \
        format(50.1584874405, predictions.mean().getrow()[0])
    assert abs(0.909348917216 - predictions.min()) < 0.0041, "expected prediction min to be {0}, but got {1}". \
        format(0.909348917216, predictions.min())
    assert abs(393.788225427 - predictions.max()) < 1.13, "expected prediction max to be {0}, but got {1}". \
